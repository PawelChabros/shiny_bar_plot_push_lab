...
function scales() {
  ...
  let xDomain = [...new Set(data.map(d => d.year))],
      cDomain = [...new Set(data.map(d => d.group))],
      yearSum = d3.nest()
...
function stackCalc(data) {
  ...
  let newData = [],
      types = [...new Set(data.map(d => d.group))],
      x0;
  ...
  data.forEach((d, i, a) => {
    ...
    if (d.group == types[0]) {
      ...
      let fRow = newData.filter(function(d) {
        return d.year == cyear & d.group == types[types.indexOf(cGroup) - 1]
      })[0]
      x0 = fRow.n + fRow.x0;
    }
...
function labPosCalc(data) {
  ...
  let labPos = data.map(d => ({
    rX: xScale(d.year) + xScale.bandwidth() / 2 - (nChar(d.n) + 1) * 4,
    rY: yScale(d.x0 + d.n / 2) - size / 1.666,
    rW: (nChar(d.n) + 1) * 8,
    rF: cScale(d.group),
    tX: xScale(d.year) + xScale.bandwidth() / 2,
  ...
  for (let o = 0; o < 10; o++) {  
    ...
    for (let i = 1; i < labPos.length; i++) {
      ...
      if (labPos[i-1].year == labPos[i].year) {
